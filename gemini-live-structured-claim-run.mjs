...
const receipt = {
  gate: 'ALPHA_NSH_STRUCTURED_CLAIM_VECTOR_AND_ADJUDICATION_V1', status: 'HOLD', claimSchema: STRUCTURED_CLAIM_SCHEMA,
  packetSha256: bind.packetSha256, model: MODEL, freeTierAuthorized: auth.authorized === true,
  keyConfigured: Boolean(process.env.GEMINI_API_KEY), authenticatedModelSurface: false, liveInferenceExecuted: false,
  zeroCorpusWrites: true, authorityMutation: false, pointerPromotion: false, globalBind: false, externalActuation: false
};
...
if (auth.modelTarget !== MODEL || auth.authorized !== true) { receipt.status = 'HOLD_STRUCTURED_CLAIM_MODEL_AUTHORIZATION'; console.log(JSON.stringify(receipt)); process.exit(0); }
if (!process.env.GEMINI_API_KEY) { receipt.status = 'HOLD_GEMINI_API_KEY_NOT_CONFIGURED_IN_GITHUB_ACTIONS'; console.log(JSON.stringify(receipt)); process.exit(0); }
...
if (!modelsResponse.ok) { receipt.status = `HOLD_GEMINI_AUTHENTICATED_MODEL_SURFACE_HTTP_${modelsResponse.status}`; console.log(JSON.stringify(receipt)); process.exit(0); }
const models = await modelsResponse.json();
receipt.authenticatedModelSurface = (models.models || []).map(m => String(m.name || '').replace(/^models\//, '')).includes(MODEL);
if (!receipt.authenticatedModelSurface) { receipt.status = 'HOLD_GEMINI_3_7_FLASH_NOT_AVAILABLE_TO_CONFIGURED_KEY'; console.log(JSON.stringify(receipt)); process.exit(0); }
...
if (!response.ok) { receipt.status = `HOLD_GEMINI_STRUCTURED_CLAIMS_HTTP_${response.status}`; console.log(JSON.stringify(receipt)); process.exit(0); }
const payload = await response.json();
const text = payload?.candidates?.[0]?.content?.parts?.[0]?.text;
if (!text) { receipt.status = 'HOLD_GEMINI_STRUCTURED_CLAIMS_EMPTY'; console.log(JSON.stringify(receipt)); process.exit(0); }
let candidate;
try { candidate = JSON.parse(text); } catch { receipt.status = 'HOLD_GEMINI_STRUCTURED_CLAIMS_MALFORMED_JSON'; console.log(JSON.stringify(receipt)); process.exit(0); }
try { receipt.run = normalizeStructuredClaimRun({ consumer: 'GEMINI', ...candidate }, bind); }
catch (error) { receipt.status = `HOLD_GEMINI_STRUCTURED_CLAIM_CONTRACT_REJECTED__${String(error?.message || 'unknown').replace(/\s+/g,'_')}`; console.log(JSON.stringify(receipt)); process.exit(0); }
